...
routes.router.post('/editTool', routes.checkAdmin, async (req, res) => {
  ...
  try {
    let sql1 = "UPDATE singleTools SET toolName = ? WHERE toolId = ? AND shopId = ?";
    await db.awaitQuery(sql1, [req.body.name, req.body.id, req.user.shopId]);

    let sql2 = "UPDATE toolHistory SET toolName = ? WHERE toolId = ? AND shopId = ?";
    await db.awaitQuery(sql2, [req.body.name, req.body.id, req.user.shopId]);
